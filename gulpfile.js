const gulp          = require('gulp'),                     // The streaming build system
      ...
      browserify    = require('gulp-browserify'),          // Bundle modules with Browserify
      ...
      include       = require('gulp-include'),             // Makes inclusion of files a breeze. Enables functionality similar to that of snockets / sprockets or other file insertion compilation tools.
...
const processors = [
  ...
  // You need uncomment this block, if you want remove unused styles from CSS
  // require('postcss-uncss')({
  //   html: ['./dist/*.html'],   //  List of html files to parse for selectors and elements
  //   ignore: ['.example-class'] //  List of selectors that should not be removed by UnCSS
  // })
];
...
gulp.task('css:fonts', () => {
  ...
  return gulp.src(path.app.scss.files.fonts) // Gulp get the file from '.../scss/fonts.scss'
    // Prevent pipe breaking caused by errors from gulp plugins
    .pipe(plumber({
      errorHandler: notify.onError((err) => {
        return {
    ...
    .pipe(production(combine(cleanCSS(), rev()))) // If it's production this plugin will minify css and write manifest
    .pipe(gulp.dest(path.dist.css.folder)) // Gulp unloads files in './dist/css/'
    .pipe(production(combine(rev.manifest('fonts-css.json'), gulp.dest(path.app.manifest.folder)))) // If it's production this plugin will write manifest in fonts-css.json and unload it to './app/manifest/'
    .pipe(browserSync.reload({stream: true})); // Browser will reload
...
gulp.task('img', () => {
  ...
  return gulp.src(path.app.img.allImg, {since: gulp.lastRun('img')}) // Gulp get the file from './app/*.{png,jpg}'
    // Prevent pipe breaking caused by errors from gulp plugins
    .pipe(plumber({
    errorHandler: notify.onError((err) => {
      return {
...
gulp.task('fonts', () => {
  ...
  return gulp.src(path.app.fonts.allFiles)
    .pipe(gulp.dest(path.dist.fonts.folder))
    .pipe(browserSync.reload({stream: true})); // Browser will reload;
...
gulp.task(
  ...
  gulp.series(
    'env', 'del',
    gulp.parallel(
      'img:sprites', 'svg:sprites', 'htaccess', 'fonts', 'css:fonts', 'css:main', 'css:libs', 'css:header', 'img', 'webpack'
    ),
    gulp.series('jade'),
    gulp.parallel(
      'server',
      () => {
        ...
        gulp.watch(path.app.scss.files.fonts, gulp.series('css:fonts'));
        gulp.watch(path.app.fonts.allFiles, gulp.series('fonts'));
